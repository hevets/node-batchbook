...
var Client = module.exports = exports = function Client (options) {
  ...
  this.baseUrl = 'https://' + this.account + '.' + this.host;
  this.cred = '?auth_token=' + this.key;
  this.account = options.account;
  this.host = (options.host) ? options.host : 'batchbook.com/api/v1/';
...
Client.prototype.parseUrl = function(ep, data) {
  var url = this.getEndpoint(ep);

  if(!data) {
    url += '.' + this.ext;
  } else {
    url += this.parseData(data);
  }

  return this.baseUrl + url + this.cred;
};
...
// client.get('people', function() {
Client.prototype.get = function(ep, data, fn) {
  if(_.isFunction(data)) fn = data, data = null;
  var url = this.getUrl('GET', ep, data);
};
 ...
 * @api public
 */
Client.prototype.getOld = function(endpoint, options) {
 ...
 * @api public
 */
Client.prototype.postOld = function(endpoint, data) {
  if(!endpoint) throw new Error('provide a batchbook endpoint');
 ...
 * @api public
 */
Client.prototype.putOld = function(endpoint, data) {
  if(!endpoint) throw new Error('provide a batchbook endpoint');
  data = this.formatData(data);
 ...
 * @returns {Object}
 */
Client.prototype.deleteOld = function(endpoint, id) {
  id = this.formatId(id);
...
Client.prototype.getEndpoint = function(endpoint) {
 ...
 * @returns {String}
 */
Client.prototype.getUrlOld = function(endpoint, options) {
  if(!endpoint)
    throw new Error('provide an endpoint');

  var url = 'https://' + this.account + '.' + this.host + this.getEndpoint(endpoint);
...
Client.prototype.getPutUrl = function(endpoint, data) {
  ...
  var url = 'https://' + this.account + '.' + this.host + this.endpoints[endpoint];
  var cred = '?auth_token=' + this.key;
